...
#    java -Dlog4j.configurationFile=file:$javaProcessLogFile -jar ipLocationProcessor.jar --spring.config.location=$commonConfiguration,$javaProcessPropertyFile 1>/dev/null 2>${log_path}/${module_name}_$ipType.error
      ...
      java -Dlog.level=${log_level} -Dlog.path=${log_path} -Dmodule.name=${ipType}  -Dlog4j.configurationFile=file:$javaProcessLogFile  -Dspring.config.location=file:$javaProcessPropertyFile,file:${commonConfiguration} -jar ${build} $ipType  1>/dev/null 2>${log_path}/$ipType.error
