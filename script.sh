...
# Script Starts : 
   ...
   if [ "$ipType" == "ipv4" ]; then
       ipFileName=${ipv4_FileName}
   else 
       ipFileName=${ipv6_FileName}
   fi
    echo "File name   ${ipFileName}"
    ...
    errorFileProcessedPath="${errorFileProcessedPath/<ipType>/$ipType}"

    mkdir -p $inputFilePath
    mkdir -p $processedFilePath
    mkdir -p $deltaFileProcessedPath
    mkdir -p $errorFileProcessedPath
    ...
    fileCopyApi=$(get_value "eirs.filecopy.url");
    ...
    insert into modules_audit_trail (status_code,status,error_message,feature_name,server_name,execution_time,module_name,info)
...
#        log_message "Moved file ${sortedTempFile} to ${processedFilePath}/${fileName}."
    ...
    mkdir -p $log_path
    sleep 45
### DMC added error logs ###

#    java -Dlog4j.configurationFile=file:$javaProcessLogFile -jar ipLocationProcessor.jar --spring.config.location=$commonConfiguration,$javaProcessPropertyFile 1>/dev/null 2>${log_path}/${module_name}_$ipType.error

      java -Dlog.level=${log_level} -Dlog.path=${log_path} -Dmodule.name=${ipType}  -Dlog4j.configurationFile=file:$javaProcessLogFile  -Dspring.config.location=file:$javaProcessPropertyFile,file:${commonConfigurationFile} -jar ${build} $ipType  1>/dev/null 2>${log_path}/$ipType.error
