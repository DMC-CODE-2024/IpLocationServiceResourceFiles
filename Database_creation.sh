...
insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values ( 'The key used for api call to ip location database to query country for an IP.', 'ipCountryApiKey', '63E2F05CC085E319664B99D6AB5D4426', 'Ip location' , 0,1,'','','');
insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values ( 'The url used for api call to ip location database to query country for an IP.', 'ipCountryApiUrl', 'https://api.ip2location.io/?key=<key>&ip=<ip>&format=json', 'Ip location', 0,1,'','','' );

insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values('The URL used to download IP location dump.', 'ipLocationDumpFileURL', 'https://www.ip2location.com/download?token=<token>\\&file=<code>', 'Ip Location' , 0,1,'','','' );
insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values( 'The code used to download ipv4 ip location dump.', 'ipLocationCodeipv4', 'DB1', 'Ip Location' , 0,1,'','','' );
insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values( 'The code used to download ipv6 ip location dump', 'ipLocationCodeipv6', 'DB1IPV6', 'Ip Location' , 0,1,'','','');
insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values( 'The tag is used to store the last processed date for ip location processor for ip-type ipv6.', 'last_process_date_ip_location_ipv6', '', 'Ip Location', 0,1,'','','');

insert into sys_param (description, tag, value, feature_name,type,active,remark,user_type,modified_by) values( 'The tag is used to store the last processed date for ip location processor for ip-type ipv4.', 'last_process_date_ip_location_ipv4', '', 'Ip Location' , 0,1,'','','');
